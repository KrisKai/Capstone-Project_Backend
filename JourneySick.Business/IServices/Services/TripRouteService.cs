...
namespace JourneySick.Business.IServices.Services
{
    public class TripRouteService : ITripRouteService
    {
        ...
        public async Task<AllTripRouteDTO> GetAllTripRoutesWithPaging(int pageIndex, int pageSize, string? routeId, string tripId, DateTime? planDateTime)
        {
        ...
        public async Task<int> DeleteTripRoute(int routeId)
        {
            try
            {
                ...
                if (getTrip != null)
                {
                    if (await _tripRouteRepository.DeleteTripRoute(routeId) > 0 && await _mapLocationRepository.DeleteMapLocation((int)getTrip.MapId) > 0)
                    {
                        return 1;
                    }
                    else
